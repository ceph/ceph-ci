...
from common import create_user, boto_connect, connect_with_retry, \
    make_compressible_body, object_stat, wait_for_transition, \
    get_compression_type, get_crypt_mode, has_crypt_attr, is_aead_crypt_mode
...
aes-256-gcm, restarts the gateway, then runs the transition phase.
Objects written in the put phase therefore predate the cipher change,
which is what an operator changing the cipher on an existing cluster has.
...
def run_put_phase():
    ...
    body = make_compressible_body(OBJECT_SIZE)
...
def run_transition_phase(expected_mode):
    ...
    for key in (SAME_KEY, RECOMP_KEY):
        ...
        stat = wait_for_transition(BUCKET_NAME, key, TRANSITIONS[key])
    ...
    stat = wait_for_transition(BUCKET_NAME, SSEC_KEY, TRANSITIONS[SSEC_KEY])
    mode = get_crypt_mode(stat)
    assert mode == 'SSE-C-AES256', \
    ...
    stat = wait_for_transition(BUCKET_NAME, PLAIN_KEY, TRANSITIONS[PLAIN_KEY])
    mode = get_crypt_mode(stat)
