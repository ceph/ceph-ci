...
def run_put_phase():
    ...
    put_object(client, SMALL_KEY, make_compressible_body(SMALL_SIZE), KMS_ARGS)

    log.info(f'uploading {LARGE_KEY} with sse-kms, multipart')
    put_object(client, LARGE_KEY, make_compressible_body(LARGE_SIZE), KMS_ARGS)

    log.info(f'uploading {INPLACE_KEY} with sse-kms, multipart')
    put_object(client, INPLACE_KEY, make_compressible_body(INPLACE_SIZE), KMS_ARGS)

    log.info(f'uploading {SSEC_KEY} with sse-c')
    put_object(client, SSEC_KEY, make_compressible_body(SMALL_SIZE), SSEC_ARGS)

    for key in (SMALL_KEY, LARGE_KEY, INPLACE_KEY):
        verify_encrypted(key, 'SSE-KMS')
    verify_encrypted(SSEC_KEY, 'SSE-C-AES256')
...
def run_copy_phase():
    ...
    small_body = make_compressible_body(SMALL_SIZE)
    large_body = make_compressible_body(LARGE_SIZE)
    ssec_body = make_compressible_body(SMALL_SIZE)
    ...
    inplace_body = make_compressible_body(INPLACE_SIZE)
