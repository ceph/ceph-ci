    ...
    try:
        ...
        for spec in tests:
            ...
            for workunit in to_run:
                ...
                tmp_keyring = f'{testdir}/workunit.client.{id_}.keyring'
                key_data = remote.read_file(client_keyring) + b'\n' + remote.read_file(admin_keyring)
                remote.write_file(tmp_keyring, key_data)
                args = [
                    'mkdir', '-p', '--', scratch_tmp,
                    run.Raw('&&'),
                    'cd', '--', scratch_tmp,
