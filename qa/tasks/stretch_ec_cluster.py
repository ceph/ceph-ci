...
class TestStretchCluster(MgrTestCase):
    ...
    def _setup_pool(self, pool_name, size=None, min_size=None, rule=None, erasure=False):
        ...
        """
        if erasure:
            self.mgr_cluster.mon_manager.create_pool(pool_name, min_size=min_size, erasure=erasure, num_zones=2)
    ...
    def _write_some_data(self, t, pool_name):
        ...
        args = [
            "rados", "-p", pool_name, "bench", str(t), "write", "-t", "16"]
    ...
    def test_set_stretch_ec_pool_io_when_down(self):
        ...
        # Fail over one mon in DC2. Should all be active since above min size
        ...
        # Fail over  one mon in DC2. Should all be 100% inactive since below min size
        self._fail_over_one_osd_from_dc(dc='dc2')
        self.wait_until_true_and_hold(lambda: self._pg_all_unavailable(),
                                      timeout=self.RECOVERY_PERIOD,
                                      success_hold_time=self.SUCCESS_HOLD_TIME)

        # Bring back osds in DC1 expects PGs to be 100% inactive since we have stale data
        self._bring_back_all_osds_in_dc('dc1')
        self.wait_until_true_and_hold(
