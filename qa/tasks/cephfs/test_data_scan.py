...
class Workload(object):
    ...
    def is_damaged(self):
        def is_marked_damaged(rank):
            mds_map = self._filesystem.get_mds_map()
            return rank in mds_map['damaged']

        self.wait_until_true(lambda: is_marked_damaged(0), 60)

        def get_state(mds_id):
            info = self._filesystem.get_mds_info(mds_id)
            return info['state'] if info is not None else None
        for mds_id in self._filesystem.mds_ids:
