...
class TestClientLimits(CephFSTestCase):
    ...
    def test_write_overrides_futuristic_mtime(self):
        ...
        future_mtime = int(self.mount_a.run_shell(["stat", "-c", "%Y", test_file]).stdout.getvalue().strip())
        current_time = int(self.mount_a.run_shell(["date", "+%s"]).stdout.getvalue().strip())
        self.assertTrue(future_mtime > current_time, "Setup error: File mtime is not in the future.")

        # Read the file from Client 2 (mount_b) to break Client 1's exclusive caps (EXCL)
        log.info("Opening file on client_b to force cap downgrade (revoking EXCL) on client_a")
        proc_b = self.mount_b.run_shell_background(["cat", test_file])
        ...
        post_mtime = int(self.mount_a.run_shell(["stat", "-c", "%Y", test_file]).stdout.getvalue().strip())

        # Refresh current time gauge
        final_current_time = int(self.mount_a.run_shell(["date", "+%s"]).stdout.getvalue().strip())

        # Clean up background process contexts and testing assets
        proc_b.wait()
        self.mount_a.run_shell(["rm", "-rf", test_dir])

        # Final Assertion Checklist
        log.info(f"Future mtime: {future_mtime} | Post-write mtime: {post_mtime} | Current system time: {final_current_time}")

        # The post-write mtime should no longer be stuck in the future.
        # It must be within a safe delta (e.g., 10 seconds) of the current real system clock time.
        self.assertLessEqual(
            post_mtime,
            final_current_time + 10,
            "REGRESSION DETECTED: The futuristic mtime remained stuck after a write operation "
            "because metadata caps were not dirtied or forced backward to the real clock."
        )
