...
class TestMirroring(CephFSTestCase):
    ...
    def test_cephfs_mirror_sync_latest_snapshot(self):
        """
        That mirroring syncs only the latest snapshot and avoids the snapshots in between.
        """
        log.debug('reconfigure client auth caps')
        self.get_ceph_cmd_result(
            'auth', 'caps', "client.{0}".format(self.mount_b.client_id),
        ...
        snap_a = "snap_a"
        self.mount_a.run_shell(['mkdir', '-p', f'{dir_name}/d1'])
        self.mount_a.write_n_mb(os.path.join(f'{dir_name}/d1', 'file1'), 3072)
        self.mount_a.run_shell(['mkdir', f'{dir_name}/.snap/{snap_a}'])
