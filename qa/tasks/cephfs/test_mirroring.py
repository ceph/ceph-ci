...
class TestMirroring(CephFSTestCase):
    ...
    def test_cephfs_mirror_survives_corrupt_sync_stat_omap(self):
        ...
        Stop the daemon, rewrite last_synced_snap fields with string types
        that used to trip json_spirit getters (and the dangling-ref load
        path), then restart. The daemon should come back and keep syncing.
        """
        self.setup_mount_b(mds_perm='rw')
        self.enable_mirroring(self.primary_fs_name, self.primary_fs_id)
        peer_spec = "client.mirror_remote@ceph"
        ...
        # Intentionally corrupt types under last_synced_snap. Pre-fix this
        # aborted in apply_persisted_dir_sync_stat() on directory acquire.
        last = dict(stat['last_synced_snap'])
        last['id'] = str(last.get('id', 1))
        last['sync_duration'] = '59s'
        last['sync_time_stamp'] = '2069394.318493s'
        last['sync_bytes'] = '6.00 GiB'
