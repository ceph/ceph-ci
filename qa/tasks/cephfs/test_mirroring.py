...
class TestMirroring(CephFSTestCase):
    ...
    @retry_assert(timeout=1200, interval=5)
    ...
    def test_checkpoint_add_directory_notify_perf_counter(self):
        ...
        self.config_set('mds', 'mds_max_snaps_per_dir', snap_count)
        ...
        for i in range(snap_count):
            ...
            if i > 0 and i % 100 == 0:
                log.info('file/snap/checkpoint progress: %d/%d', i, snap_count)

        # Expect 1001 add_directory calls: one for the initial mirror add above
        # and one per checkpoint add (each sends an acquire notify).
        self.wait_peer_add_directory_counter(1 + snap_count)
