...
class TestMirroring(CephFSTestCase):
    ...
    def add_directory(self, fs_name, fs_id, dir_name, check_perf_counter=True,
        ...
        if sync_latest_snapshot:
            self.run_ceph_cmd("fs", "snapshot", "mirror", "add", fs_name, dir_name, "--sync-latest-snapshot")
        elif sync_from_snap_name:
            self.run_ceph_cmd("fs", "snapshot", "mirror", "add", fs_name, dir_name, sync_from_snap_name)
        else:
    ...
    def test_cephfs_mirror_sync_latest_snapshot_existing(self):
        ...
        try:
            ...
            raise RuntimeError('Expected verify_snapshot to fail')
        try:
            self.verify_snapshot(dir_name, snap_b)
        except AssertionError:
            pass
    ...
    def test_cephfs_mirror_sync_latest_snapshot_busy(self):
        ...
        snap_a = "snap_a"
        self.mount_a.run_shell(['mkdir', '-p', f'{dir_name}/d1'])
        self.mount_a.write_n_mb(os.path.join(f'{dir_name}/d1', 'file1'), 20480)
        self.mount_a.run_shell(['mkdir', f'{dir_name}/.snap/{snap_a}'])
