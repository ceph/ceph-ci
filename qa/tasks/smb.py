...
def _smb_daemon_hostnames(admin_remote, cluster_id):
    out = StringIO()
    admin_remote.run(
        args=[
            'sudo', 'ceph', 'orch', 'ps',
            '--daemon-type', 'smb', '--format', 'json',
        ],
        stdout=out,
    )
...
def power_off_smb_daemon_host(ctx, config):
    ...
    hostnames = _smb_daemon_hostnames(admin_remote, cluster_id)
