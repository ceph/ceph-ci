...
def get_obj_in_pg(manager, pool, pgid):
    """Return the name of an object that lives in the given PG.

    The admin-socket test-ops hooks (truncobj, setomapval, etc.) derive the
    target PG from the object's hash.  We therefore must pass an object that
    actually lives in *pgid*; otherwise the corruption lands on the wrong PG
    and the subsequent deep-scrub of pgid finds nothing wrong.

    'rados --pgid <pgid> ls' seeks the iterator to the start of the PG's hash
    range and the client breaks on the first object outside the range, so only
    objects belonging to *pgid* are returned.
    """
    out = manager.do_rados(['--pgid', pgid, 'ls'], pool=pool, stdout=StringIO())
    for line in out.stdout.getvalue().splitlines():
        name = line.strip()
        if name:
            return name
    raise RuntimeError('No objects found in PG %s of pool %s' % (pgid, pool))
...
class MessUp:
    ...
    def append(self):
        ...
        size = self.manager.do_rados(['stat', self.obj], pool=self.pool,
                                     stdout=StringIO())
        # rados stat output: "pool/obj mtime TIME, size BYTES"
        # split on ", size " to robustly get the byte count
        obj_size = int(size.stdout.getvalue().split(', size ')[1].strip())
        self.manager.osd_admin_socket(
...
def task(ctx, config):
    ...
    for p in osd_dump['pools']:
        if p['pool_name'] == 'rbd':
            ...
            break
    if poolid == -1:
        # cephadm deployments do not create an rbd pool automatically
        manager.create_pool('rbd', pg_num=8)
        osd_dump = manager.get_osd_dump_json()
        for p in osd_dump['pools']:
            if p['pool_name'] == 'rbd':
                poolid = p['pool']
                break
        assert poolid != -1, 'rbd pool creation failed'
    ...
    obj_name = get_obj_in_pg(manager, 'rbd', pg)
