...
def broker_conf(ctx, client, kafka_dir, kerberos, kraft):
    ...
    if kraft:
        ...
        # --standalone writes the voter set into the bootstrap metadata;
        # Kafka 4.1.1+ rejects the format command if
        # controller.quorum.voters is also set here.
        consensus_config = "controller.listener.names=CONTROLLER\n"
    else:
