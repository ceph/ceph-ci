...
function TEST_truncated_sna_record() {
    ...
    until grep -a -q "scrub_finish shard" $dir/osd.$osd.log ; do
        sleep 0.2
        waited=$(( waited + 1 ))
        if (( waited > wait_max * 5 )); then
            echo "ERROR: timed out waiting for scrub_finish shard in osd.$osd.log"
            return 1
        fi
    done
    ...
    sleep 5
    # Verify the initial clean scrub produced no snap-mapper error messages.
    # Use the specific sentinel emitted by ScrubSnapMapperRepair::run rather than
    # the generic "ERR" substring, which also matches unrelated startup ERROR lines
    # (e.g. "Falling back to public interface", "do_list_snaps: ...").
    ! grep -a -q "found snap mapper error" $dir/osd.$osd.log || return 1
    ...
    # Wait for all PGs to become active+clean after OSD restart before scrubbing.
    ...
    # Count scrub_finish occurrences already in the log before we trigger the
    # new scrub, so we can detect exactly one new completion below.
    local scrub_cnt_before
    scrub_cnt_before=$(grep -a -c "scrub_finish shard" $dir/osd.$cur_prim.log 2>/dev/null || true)

    ceph pg $pgid deep-scrub || return 1

    # Wait for the post-corruption scrub to finish (same sentinel as the first scrub).
    waited=0
    until (( $(grep -a -c "scrub_finish shard" $dir/osd.$cur_prim.log 2>/dev/null || true) > scrub_cnt_before )); do
        sleep 0.2
        waited=$(( waited + 1 ))
        if (( waited > wait_max * 5 )); then
            echo "ERROR: timed out waiting for post-corruption scrub_finish shard"
            return 1
        fi
    done

    ceph pg dump pgs
    (( extr_dbg >= 1 )) && grep -a "found snap mapper error" $dir/osd.$cur_prim.log
    grep -a -q "found snap mapper error" $dir/osd.$cur_prim.log || return 1

    # but did we fix the snap issue? let's try scrubbing again

    local prev_err_cnt=`grep -a -c "found snap mapper error" $dir/osd.$cur_prim.log`
    echo "prev count: $prev_err_cnt"

    # Count scrub_finish occurrences again before the second post-corruption scrub.
    local scrub_cnt_before2
    scrub_cnt_before2=$(grep -a -c "scrub_finish shard" $dir/osd.$cur_prim.log 2>/dev/null || true)

    # scrub again. No errors expected this time
    ceph pg $pgid deep-scrub || return 1

    # Wait for the second post-corruption scrub to finish.
    waited=0
    until (( $(grep -a -c "scrub_finish shard" $dir/osd.$cur_prim.log 2>/dev/null || true) > scrub_cnt_before2 )); do
        sleep 0.2
        waited=$(( waited + 1 ))
        if (( waited > wait_max * 5 )); then
            echo "ERROR: timed out waiting for second post-corruption scrub_finish shard"
            return 1
        fi
    done
    ...
    ceph pg dump pgs
    (( extr_dbg >= 1 )) && grep -a "found snap mapper error" $dir/osd.$cur_prim.log
    local current_err_cnt=`grep -a -c "found snap mapper error" $dir/osd.$cur_prim.log`
