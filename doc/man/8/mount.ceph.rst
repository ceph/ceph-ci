...
So, the mount command can be used as::
  ...
  mount.ceph name@fs_name=/ /mnt/mycephfs -o mon_addr=1.2.3.4
...
``mon_addr`` mount option. Multiple monitor addresses can be passed by separating
...
Pass secret key for CephX user optionally::
    ...
    mount.ceph fs_user@mycephfs2=/ /mnt/mycephfs -o mon_addr=192.168.0.1/192.168.0.2/192.168.0.3,secret=AQATSKdNGBnwLhAAnNDKnH65FmVKpXZJVasUeQ==

Pass file containing secret key to avoid leaving secret key in shell's command
history::

    mount.ceph fs_user@mycephfs2=/ /mnt/mycephfs -o mon_addr=192.168.0.1/192.168.0.2/192.168.0.3,secretfile=/etc/ceph/fs_username.secret
