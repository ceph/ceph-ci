...
rather than against the metadata document. (See: `Pre-Filtering and
Post-Filtering`_.)
...
- ``returnMetadata``: If "true", the vector metadata is returned. (This is
  "false" by default.)
- ``segmentCount``: Split the listing into this many segments, so that they may
  be listed in parallel. Must be between 1 and 16.
- ``segmentIndex``: The segment to list, between 0 and ``segmentCount`` - 1.
  Requires ``segmentCount``.
