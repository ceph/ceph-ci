...
Multisite
`````````
In a multi-zone deployment, each zone is going to use its own log object before the log object is added to the log bucket.
After the log object is added to the log bucket (e.g after being flushed) it will be replicated to the other zones.
