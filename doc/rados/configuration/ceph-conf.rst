...
#. ``service:service-id`` (or the full orchestrator service name) where the value
   ...
   Create-time options (for example ``bluestore_min_alloc_size``) are also written
   into the temporary ceph.conf used by ``ceph-volume`` / ``ceph-osd --mkfs`` so
   they take effect when the OSD is created.
