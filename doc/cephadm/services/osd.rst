...
However, users now have the option to create an OSD and directly attach it to an existing managed service by specifying the desired service name.
  ...
  .. prompt:: bash #
      ...
      ceph orch daemon add osd <host>:<device-path> [--service-name <service_name>] [--force]

  For example:

  .. prompt:: bash #

      ceph orch daemon add osd host1:/dev/sdb --service-name osd.my_custom_service --force
