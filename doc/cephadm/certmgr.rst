...
self-signed certificates generated within Cephadm. For services that require
SSL, users have the option to either bring their own certificate or allow
Cephadm to generate a self-signed certificate. This ensures secure communication
while offering flexibility for different deployment preferences.
...
- **`mgr/cephadm/certificate_automated_rotation_enabled`** (default: `False`):
  ...
  expired ones it will issue a health error/warning when some issue is detected.
...
3. **Per-Service Scope:**
   ...
   - Example: `rgw` service certificates is assigned specifically to an RGW service.


Reloading Certificate Manager
=============================

Run the following command to reload the certificate manager configuration from mon-store:
...
Replace `<certificate-name>` with the actual certificate name from ceph `ceph
...
Replace `<key-name>` with the actual certificate name from ceph `ceph
...
certain service. Replace `<entiy-name>` with the actual certificate name from ceph
`ceph orch certmgr entity ls`.
...
Use this command to upload or replace an existing certificate.
...
This command provisions new certificates for specified internal module.
