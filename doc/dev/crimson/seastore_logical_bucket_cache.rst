...
mainly for using ssd/nvme devices as non-volatile cache for the
relatively slow HDD devices. Specifically, it serves to:

#. Load the frequently accesses data from the cold tier to the Logical
   Bucket Cache.
#. Evict the relatively cold data from the Logical Bucket Cache to
   the cold tier based on the heat of both data read and write.
...
#. Extents that are loaded from the cold tier to the Logical Bucket Cache
   ...
   one in the Logical Bucket Cache, the other in the cold tier. We call
   paddrs corresponding to the cold tier shadow paddrs. So lba mappings
   have two paddrs now, the primary paddr and the shadow paddr.
