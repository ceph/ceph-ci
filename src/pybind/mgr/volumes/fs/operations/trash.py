            ...
            while de:
                ...
                try:
                    if de.is_dir():
                        ...
                        #file_count += int(self.fs.getxattr(de_path,
                        #                                   'ceph.dir.rfiles'))
                        dir_found = True
                        subvol_count += 1

                        # every subvolume has a .meta file and that is included
                        # in the count, in the value of xattr rfiles. therefore,
                        # for no need to count it now.
                        # file_count += 1
...
def get_trashcan_stats(volclient, volname):
    with open_volume_lockless(volclient, volname) as fs:
        trashcan = Trash(fs, volclient.volspec)
