...
def test_pool_missing_creates_then_enables(monkeypatch):
    ...
    assert app_enable_calls[0].get("app") == "nvmeof", "Expected 'nvmeof' application to be enabled"
    assert app_enable_calls[0].get("pool") == ".nvmeof", "Expected pool to be '.nvmeof'"


def test_enable_application_calls_appify_pool_with_correct_params(monkeypatch):
    calls = []
    ...
    mgr._enable_application(".nvmeof", "nvmeof")

    nvmeof_enable_calls = [c for c in calls if c.get("prefix") == "osd pool application enable" and c.get("app") == "nvmeof"]
    assert len(nvmeof_enable_calls) == 1, "Expected one 'nvmeof' application enable call"
    assert nvmeof_enable_calls[0].get("pool") == ".nvmeof"
