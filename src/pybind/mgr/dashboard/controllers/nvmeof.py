...
try:
    ...
    class NVMeoFSpdk(RESTController):
        ...
        @map_model(model.RequestStatus)
        @handle_nvmeof_error
        def set_log_level(self, log_level: Optional[str]=None, print_level: Optional[str]=None, gw_group: Optional[str] = None):
            ...
            spdk_log_level = NVMeoFClient(gw_group=gw_group).stub.set_gateway_log_level(
                NVMeoFClient.pb2.set_spdk_nvmf_logs_req(log_level=log_level, print_level=print_level)
            )
            return spdk_log_level
        ...
        @map_model(model.RequestStatus)
        @handle_nvmeof_error
        def set_log_level(self, gw_group: Optional[str] = None):
