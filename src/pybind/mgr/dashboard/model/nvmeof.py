...
class CliEmptyMessage:
    ...
    - Any field from the parent NamedTuple (e.g., {subsystem_nqn}, {nqn})
...
class SubsystemList(NamedTuple):
    status: int
    error_message: str
    subsystems: Annotated[List[Subsystem], CliFlags.EXCLUSIVE_LIST, CliEmptyMessage("No subsystems")]
...
class ConnectionList(NamedTuple):
    ...
    error_message: str
    subsystem_nqn: str
    connections: Annotated[List[Connection], CliFlags.EXCLUSIVE_LIST, CliEmptyMessage("No connections for {subsystem_nqn}")]
...
class ListenerList(NamedTuple):
    status: int
    error_message: str
    listeners: Annotated[List[Listener], CliFlags.EXCLUSIVE_LIST, CliEmptyMessage("No listeners for {nqn}")]
    nqn: Annotated[str, CliFlags.DROP] = ""  # Injected by pick decorator, not from protobuf
...
class HostsInfo(NamedTuple):
    ...
    allow_any_host: bool
    subsystem_nqn: str
    hosts: Annotated[List[Host], CliFlags.EXCLUSIVE_LIST, CliEmptyMessage("No hosts are allowed to access {subsystem_nqn}")]
