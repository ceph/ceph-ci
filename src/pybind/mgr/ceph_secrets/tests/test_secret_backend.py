...
class TestSecretStorageBackend:
    ...
    def test_full_implementation_is_instantiable(self):
        class FullImpl(SecretStorageBackend):
            def get(self, ns, scope, target, name): return None
            def set(self, ns, scope, target, name, data, user_made=True, editable=True): ...
            def rm(self, ns, scope, target, name): return False
            def ls(self, namespace=None, scope=None, target=None): return []
            def get_epoch(self, namespace): return 0
            def bump_epoch(self, namespace): return 1
