...
class TestSecretRecord:
    ...
    def test_basic_construction(self):
        rec = SecretRecord(ref=self._ref(), metadata=self._meta(), data={"v": "s3cr3t"})
    ...
    def test_to_public_json_with_data(self):
        ...
        assert out["data"]["v"] == "s3cr3t"

    def test_to_public_json_with_ref(self):
        rec = SecretRecord(ref=self._ref(), metadata=self._meta(), data={})
    ...
    def test_to_store_json(self):
        rec = SecretRecord(ref=self._ref(), metadata=self._meta(), data={"x": 1})
    ...
    def test_from_store_json_roundtrip(self):
        ref = self._ref()
        rec = SecretRecord(ref=ref, metadata=self._meta(), data={"v": "pw"})
        rec2 = SecretRecord.from_store_json(ref, rec.to_store_json())
        assert rec2.data["v"] == "pw"
    ...
    def test_bad_ref_type_raises(self):
        with pytest.raises(CephSecretDataError, match="must be a SecretRef"):
            SecretRecord(ref="not-a-ref", metadata=self._meta(), data={})  # type: ignore[arg-type]

    def test_bad_metadata_type_raises(self):
        with pytest.raises(CephSecretDataError):
            SecretRecord(ref=self._ref(), metadata={"version": 1}, data={})  # type: ignore[arg-type]

    def test_bad_data_type_raises(self):
        with pytest.raises(CephSecretDataError):
            SecretRecord(ref=self._ref(), metadata=self._meta(), data=["not", "a", "dict"])  # type: ignore[arg-type]

    def test_data_is_deep_copied_in_store_json(self):
        rec = SecretRecord(ref=self._ref(), metadata=self._meta(), data={"k": "v"})
        stored = rec.to_store_json()
        stored["data"]["k"] = "mutated"
        # to_store_json makes a deep copy; original record data is unchanged
        assert rec.data["k"] == "v"

    def test_ident(self):
        ref = self._ref()
        rec = SecretRecord(ref=ref, metadata=self._meta(), data={})
...
class TestSecretStoreMon_Ls:
    ...
    def test_ls_returns_all(self, store):
        store.set("ns", SecretScope.GLOBAL, "", "k1", {"v": "1"})
        store.set("ns", SecretScope.GLOBAL, "", "k2", {"v": "2"})
        recs = store.ls(namespace="ns")
        assert len(recs) == 2

    def test_ls_filter_scope(self, store):
        store.set("ns", SecretScope.GLOBAL, "", "g", {"v": "1"})
        store.set("ns", SecretScope.SERVICE, "prom", "auth", {"v": "2"})
    ...
    def test_ls_filter_target(self, store):
        store.set("ns", SecretScope.SERVICE, "prom", "a1", {"v": "1"})
        store.set("ns", SecretScope.SERVICE, "grafana", "a2", {"v": "2"})
    ...
    def test_ls_namespace_isolation(self, store):
        store.set("ns1", SecretScope.GLOBAL, "", "k", {"v": "1"})
        store.set("ns2", SecretScope.GLOBAL, "", "k", {"v": "2"})
        recs = store.ls(namespace="ns1")
        assert len(recs) == 1

    def test_ls_custom_scope(self, store):
        store.set("ns", SecretScope.CUSTOM, "", "a/b/c", {"v": "1"})
        store.set("ns", SecretScope.CUSTOM, "", "x/y", {"v": "2"})
        recs = store.ls(namespace="ns", scope=SecretScope.CUSTOM)
        assert len(recs) == 2

    def test_ls_sorted(self, store):
        store.set("ns", SecretScope.GLOBAL, "", "zz", {"v": "1"})
        store.set("ns", SecretScope.GLOBAL, "", "aa", {"v": "2"})
        recs = store.ls(namespace="ns")
    ...
    def test_ls_no_filter(self, store):
        store.set("ns1", SecretScope.GLOBAL, "", "a", {"v": "1"})
        store.set("ns2", SecretScope.GLOBAL, "", "b", {"v": "2"})
        recs = store.ls()
        assert len(recs) == 2
