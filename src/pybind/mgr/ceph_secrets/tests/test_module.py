...
from ceph_secrets.module import Module, _parse_data_arg
...
from ceph_secrets_types import (
    ...
    CephSecretException, CephSecretNotFoundError, CephSecretDataError,
)
...
class TestModuleRPC:
    ...
    def test_secret_get_versions_batch(self, mod):
        mod.secret_set("ns", SecretScope.GLOBAL, "", "a", {"v": "1"})
        mod.secret_set("ns", SecretScope.GLOBAL, "", "b", {"v": "2"})
    ...
    def test_secret_set_returns_metadata(self, mod):
        result = mod.secret_set("ns", SecretScope.GLOBAL, "", "k", {"v": "x"})
    ...
    def test_secret_ls_scope_filter(self, mod):
        mod.secret_set("ns", SecretScope.GLOBAL, "", "g", {"v": "1"})
        mod.secret_set("ns", SecretScope.SERVICE, "prom", "auth", {"v": "2"})
    ...
    def test_scan_unresolved_refs_exists(self, mod):
        mod.secret_set("ns", SecretScope.GLOBAL, "", "pw", {"value": "x"})
        result = mod.scan_unresolved_refs("secret:/ns/global/pw", namespace="ns")
        assert result == []

    def test_resolve_object(self, mod):
        mod.secret_set("ns", SecretScope.GLOBAL, "", "pw", {"value": "s3cr3t"})
...
class TestModuleInternals:
    ...
    def test_secret_get_existing(self, mod):
        mod.secret_set("ns", SecretScope.GLOBAL, "", "key", {"v": "x"})
    ...
    def test_secret_set_returns_metadata_without_data(self, mod):
        result = mod._secret_set(self._ref(), {"v": "x"})
        assert "metadata" in result
        assert "data" not in result

    def test_secret_rm_existing(self, mod):
        mod._secret_set(self._ref(), {"v": "x"})
...
class TestModuleCLIHandlers:
    ...
    def test_cli_set_by_path_bad_json(self, mod):
        from object_format import ErrorResponse
        try:
            result = mod._cli_secret_set_by_path(path="ns/global/key", inbuf="{bad}")
            self._assert_error(result)
        except ErrorResponse:
            pass
    ...
    def test_cli_get_with_reveal(self, mod):
        mod._cli_secret_set_by_path(path="ns/global/pw", inbuf='{"v": "s3cr3t"}')
