...
@patch("cephadm.services.nfs.NFSService.purge", MagicMock())
@patch("cephadm.services.nfs.NFSService.create_rados_config_obj", MagicMock())
def test_ingress_haproxy_nfs_backend_sighup(cephadm_module, mock_cephadm):
    nfs_spec = NFSServiceSpec(
        service_id="foo",
        placement=PlacementSpec(hosts=['test']),
    )
    ...
    with contextlib.ExitStack() as stack:
        ...
        nfs_spec.placement = PlacementSpec(hosts=['test', 'test2'])
        cephadm_module.apply([nfs_spec])
        mock_cephadm.serve(cephadm_module)._apply_all_services()
        nfs_dds = cephadm_module.cache.get_daemons_by_service(
            ingress_spec.backend_service
        )
        nfs_dd = nfs_dds[1]
