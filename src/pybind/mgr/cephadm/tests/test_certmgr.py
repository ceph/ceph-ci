...
class TestCertMgr(object):
    ...
    def test_tlsobject_store_cert_ls(self, _set_store, cephadm_module: CephadmOrchestrator):
        ...
        def compare_certls_dicts(expected_ls):
            ...
            for svc_cert_name, value in expected_ls.items():
                if 'extensions' in value:  # case of global certificates
                    assert 'validity' in value
                    validity = value['validity']
                    assert re.match(validity['not_after'], actual_ls[svc_cert_name]['validity']['not_after'])
                    assert re.match(validity['not_before'], actual_ls[svc_cert_name]['validity']['not_before'])
                else:  # case of per service/host certificates
                    for target, cert_info in value.items():
                        assert 'validity' in cert_info
                        validity = cert_info['validity']
                        assert re.match(validity['not_after'], actual_ls[svc_cert_name][target]['validity']['not_after'])
                        assert re.match(validity['not_before'], actual_ls[svc_cert_name][target]['validity']['not_before'])
        ...
        expected_ls['grafana_cert']['host1'] = get_generated_cephadm_cert_info_1()
        expected_ls['grafana_cert']['host2'] = get_generated_cephadm_cert_info_2()
        compare_certls_dicts(expected_ls)
        ...
        expected_ls['mgmt_gw_cert'] = get_generated_cephadm_cert_info_1()
        expected_ls['oauth2_proxy_cert'] = get_generated_cephadm_cert_info_2()
        compare_certls_dicts(expected_ls)
    ...
    def test_certificate_renewal_for_self_signed(self, _set_store, cephadm_module: CephadmOrchestrator):
        ...
        with mock.patch.object(cert_mgr.ssl_certs, "renew_cert", return_value=("mock_new_cert", "mock_new_key")) as renew_mock:
            cert_mgr._renew_self_signed_certificate(cert_info, cert_obj, "test_host")
        ...
        cert_obj = Cert(EXPIRED_CERT, user_made=False)
        with mock.patch.object(cert_mgr.ssl_certs, "renew_cert", return_value=("mock_new_cert", "mock_new_key")) as renew_mock:
            cert_mgr._renew_self_signed_certificate(cert_info, cert_obj, "test_service")
            renew_mock.assert_called_once()
    ...
    def test_health_warning_on_bad_certificates(self, _set_store, cephadm_module: CephadmOrchestrator):
        ...
        # Test health warning is raised if no invalid cert is detected
        problematic_certs = [
            CertInfo("test_service", "test_target", is_valid=True, error_info="about to expire"),
        ]
...
class TestTLSObjectStore(unittest.TestCase):
    ...
    def test_get_tlsobjects(self):
        self.store.save_tlsobject("per_service1", "cert_data", service_name="my_service")
        tlsobjects = self.store.get_tlsobjects()
        self.assertIn("per_service1", tlsobjects)
