...
class OAuth2ProxyService(CephadmService):
    ...
    def get_certificates(self, svc_spec: OAuth2ProxySpec, daemon_spec: CephadmDaemonDeploySpec) -> Tuple[str, str]:
        ...
        cert = self.mgr.cert_mgr.get_cert('oauth2_proxy_cert', host=addr)
        key = self.mgr.cert_mgr.get_key('oauth2_proxy_key', host=addr)
        if not (cert and key):
            ...
            # save certificates
            if cert and key:
                self.mgr.cert_mgr.save_cert('oauth2_proxy_cert', cert, host=addr)
                self.mgr.cert_mgr.save_key('oauth2_proxy_key', key, host=addr)
            else:
    ...
    def post_remove(self, daemon: DaemonDescription, is_failed_deploy: bool) -> None:
        ...
        self.mgr.cert_mgr.rm_cert('oauth2_proxy_cert', host=daemon.hostname)
        self.mgr.cert_mgr.rm_key('oauth2_proxy_key', host=daemon.hostname)
