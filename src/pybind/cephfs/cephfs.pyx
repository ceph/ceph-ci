...
cdef class BlockDiffHandle(object):
    ...
    def readblock(self):
        ...
        with nogil:
            if self.opened:
                ret = ceph_read_blockdiff(&self.handle, &chblks)
            else:
                raise make_ex(EBADF, "dir is not open")
        if ret < 0:
    ...
    def close(self):
        ...
        self.lib.require_state("mounted")
        with nogil:
            ret = ceph_close_blockdiff(&self.handle)
        if ret < 0:
...
cdef class LibCephFS(object):
    ...
    def openblockdiff(self, root_path, rel_path, snap1name, snap2name) -> BlockDiffHandle:
        """
        Opens the blockdiff
        ...
        cdef:
            ...
            char* _snap2 = snap2
        with nogil:
            ret = ceph_open_blockdiff(self.cluster, _root, _relp, _snap1, _snap2, &h.handle);
        if ret < 0:
