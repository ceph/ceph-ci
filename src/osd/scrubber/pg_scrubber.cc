...
{
  get_counters_set().labeled_set->inc(scrbcnt_chunks_selected);
  ...
  if (num_chunk_objects.has_value()) {
    ...
    get_counters_set().labeled_set->inc(scrbcnt_chunks_busy);
  }
}
...
bool PgScrubber::write_blocked_by_scrub(const hobject_t& soid)
{
  ...
  get_counters_set().labeled_set->inc(scrbcnt_write_blocked);
				      ...
				      bool deep)
{
  ...
  // scan objects
  while (!pos.done()) {
    const auto& selected_io_counters = m_pg->pool.info.is_replicated()
                                           ? Scrub::io_counters_replicated
                                           : Scrub::io_counters_ec;
    int r = m_pg->get_pgbackend()->be_scan_list(selected_io_counters, map, pos);
