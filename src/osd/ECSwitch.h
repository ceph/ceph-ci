...
public:
  ...
  public:
    ...
    {
      if (switcher->is_optimized_unchecked()) return (*optimized)(have);
      return (*legacy)(have);
    }
  ...
  public:
    ...
    switcher(s)
    {
      legacy = s->legacy.get_is_readable_predicate(s->get_parent()->whoami_shard());
      optimized = s->optimized.get_is_readable_predicate(s->get_parent()->whoami_shard());
    }
  ...
  {
    bool r;
    if (is_optimized_unchecked()) r = optimized._handle_message(op);
    else r = legacy._handle_message(op);
  ...
  {
    if (is_optimized_unchecked()) return optimized.check_recovery_sources(osdmap);
    return legacy.check_recovery_sources(osdmap);
  }
  void on_change() override
  {
    if (is_optimized_unchecked()) optimized.on_change();
  ...
  {
    if (is_optimized_unchecked()) optimized.clear_recovery_state();
  ...
  {
    if (is_optimized()) return optimized.get_ec_data_chunk_count();
    return legacy.get_ec_data_chunk_count();
  }
  int get_ec_stripe_chunk_size() const override {
    if (is_optimized()) return optimized.get_ec_stripe_chunk_size();
    ...
    std::map<std::string, ceph::buffer::list, std::less<>> *out) override
  {
    ...
    if (r < 0)
      return r;
    if (is_optimized()) return optimized.objects_get_attrs(hoid, out);
