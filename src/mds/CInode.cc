...
void CInode::decode_lock_ipolicy(bufferlist::const_iterator& p)
{
  ...
  bool pin_updated = (get_inode()->export_pin != _inode->export_pin) ||
		     ...
		     (get_inode()->export_ephemeral_random_pin !=
		      _inode->export_ephemeral_random_pin);
  reset_inode(std::move(_inode));
...
{
  // the effective probability is clamped by mds_export_ephemeral_random_max
  double threshold = std::min(get_inode()->export_ephemeral_random_pin,
                              mdcache->export_ephemeral_random_max);
  if (threshold <= 0.0)
    return false;
  if (threshold >= 1.0)
    return true;
