...
void MDCache::quiesce_overdrive_fragmenting(CDir* dir, bool async) {
  ...
  for (auto it = fragments.lower_bound({diri->ino(), {}});
    ...
    if (it->first.frag.contains(mydf.frag)) {
      ...
      dout(10) << __func__ << ": will schedule async abort_if_freezing for " << *it->second.mdr << dendl;

      if (async) {
        mds->queue_waiter(new MDSInternalContextWrapper(mds, new LambdaContext( [this, mdr = it->second.mdr] {
          dispatch_fragment_dir(mdr, true);
        })));
      } else {
        dispatch_fragment_dir(it->second.mdr, true);
      }
