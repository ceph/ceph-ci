 ...
 * deprioritize standbys sharing a host with these addresses. If nullptr,
 * anti-affinity checking is safely bypassed.
 */
const MDSMap::mds_info_t* FSMap::get_available_standby(
    const Filesystem& fs,
    const entity_addrvec_t* avoid_addrs) const
{
  ...
  for (const auto& [gid, info] : standby_daemons) {
    ...
    // Evaluate host/IP anti-affinity
    bool same_host = false;
    if (avoid_addrs && !info.addrs.empty() && !avoid_addrs->empty()) {
      same_host = info.addrs.front().is_same_host(avoid_addrs->front());
    }
     ...
     *
     *                     | Diff Host (Pref)    | Same Host (Fallback)    |
     * --------------------|---------------------|-------------------------|
     * Exact Match (fscid) | SCORE_PREF_MATCH    | SCORE_FALLBACK_MATCH    |
     * Vanilla (NONE)      | SCORE_PREF_VANILLA  | SCORE_FALLBACK_VANILLA  |
     * Last Resort (Other) | SCORE_PREF_OTHER_FS | SCORE_FALLBACK_OTHER_FS |
     */
