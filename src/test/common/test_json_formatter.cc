...
TEST(formatter, ceph_json_direct_api)
{
  constexpr string_view json =
    R"({"name":"fast","enabled":true,"nested":{"secret":"open"},)"
    R"("values":["a","b"],"keys":[{"kid":"kid-a","x5c":["cert-a",)"
    R"("cert-b"]},{"kid":"kid-b","kty":"RSA","n":"modulus",)"
    R"("e":"AQAB"}],"large":18446744073709551615})";

  ceph_json::value root;
  ASSERT_TRUE(ceph_json::parse(json, root));
  EXPECT_EQ("fast", ceph_json::require<string>(root, "name"));
  EXPECT_TRUE(ceph_json::require<bool>(root, "enabled"));
  EXPECT_EQ("open", ceph_json::require<string>(
    ceph_json::require_object(root, "nested"), "secret"));

  const auto values = ceph_json::require_array(root, "values");
  ASSERT_EQ(2uz, values.size());
  EXPECT_EQ("a", ceph_json::decode_value<string>(values.front()));
  EXPECT_EQ("b", ceph_json::decode_value<string>(values.back()));

  const auto keys = ceph_json::require_array(root, "keys");
  ASSERT_EQ(2uz, keys.size());
  EXPECT_EQ("kid-a", ceph_json::require<string>(keys.front(), "kid"));
  const auto certificates =
    ceph_json::field<vector<string>>(keys.front(), "x5c");
  ASSERT_TRUE(certificates);
  EXPECT_EQ((vector<string> {"cert-a", "cert-b"}), *certificates);
  EXPECT_EQ(nullopt, ceph_json::field<string>(keys.back(), "x5c"));

  EXPECT_EQ(numeric_limits<uint64_t>::max(),
            ceph_json::require<uint64_t>(root, "large"));
  EXPECT_EQ(nullopt, ceph_json::field<string>(root, "missing"));

  EXPECT_THROW(ceph_json::require<string>(root, "missing"), JSONDecoder::err);
  EXPECT_THROW(ceph_json::require<int>(root, "name"), JSONDecoder::err);
  EXPECT_THROW(ceph_json::field<int>(root, "name"), JSONDecoder::err);
  EXPECT_THROW(ceph_json::require_object(root, "name"), JSONDecoder::err);
  EXPECT_THROW(ceph_json::require_array(root, "name"), JSONDecoder::err);

  ceph_json::value malformed;
  EXPECT_FALSE(ceph_json::parse(R"({"unterminated":)", malformed));
  EXPECT_THROW(ceph_json::parse(R"({"unterminated":)"), JSONDecoder::err);
...
TEST(formatter, ceph_json_object_tree_compatibility)
{
  ...
  EXPECT_TRUE(values[0] == "first" || values[1] == "first");
  EXPECT_TRUE(values[0] == "second" || values[1] == "second");
