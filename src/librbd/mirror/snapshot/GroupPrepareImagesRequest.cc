...
void GroupPrepareImagesRequest<I>::handle_list_group_images(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " error listing images in group: "
...
void GroupPrepareImagesRequest<I>::handle_check_mirror_images_disabled(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " images not disabled for mirroring: "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
void GroupPrepareImagesRequest<I>::handle_open_group_images(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " failed to open group images: "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
template <typename I>
void GroupPrepareImagesRequest<I>::check_images_mirror_mode() {
  ldout(m_cct, 10) << dendl;

  auto ctx = create_context_callback<
    GroupPrepareImagesRequest<I>,
    &GroupPrepareImagesRequest<I>::handle_check_images_mirror_mode>(this);
  ...
  for (size_t i = 0; i < m_image_ctxs.size(); i++) {
    ...
    auto on_mirror_mode_get = new LambdaContext(
      [this, i, new_sub_ctx=gather_ctx->new_sub()](int r) {
        cls::rbd::MirrorMode mirror_mode = cls::rbd::MIRROR_MODE_DISABLED;
        if (r == 0) {
          auto iter = m_out_bls[i].cbegin();
          r = cls_client::mirror_mode_get_finish(&iter, &mirror_mode);
        }
        if (r < 0) {
          lderr(m_cct) << "image_id="<< m_images[i].spec.image_id
        ...
        if (mirror_mode != cls::rbd::MIRROR_MODE_IMAGE) {
          lderr(m_cct) << "image_id=" << m_images[i].spec.image_id
                       << ", cannot disable mirroring in the current pool"
                       << " mirroring mode" << dendl;
    ...
    auto comp = create_rados_callback(on_mirror_mode_get);
    int r = m_image_ctxs[i]->md_ctx.aio_operate(RBD_MIRRORING, comp, &op,
                                                &m_out_bls[i]);
    ceph_assert(r == 0);
    comp->release();
  }
...
template <typename I>
void GroupPrepareImagesRequest<I>::handle_check_images_mirror_mode(int r) {
  ldout(m_cct, 10) << "r=" << r << dendl;
  ...
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " mirror mode validation for images failed: "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
    ...
    auto info_ctx = new LambdaContext(
      [this, i, new_sub_ctx=gather_ctx->new_sub()](int r) {
        ...
        if (m_operation != OP_DISABLE &&
            ...
            (*m_mirror_images)[i].state != cls::rbd::MIRROR_IMAGE_STATE_CREATING) {
          lderr(image_cct) << "image_id=" << m_images[i].spec.image_id
        ...
        if (m_operation == OP_DISABLE) {
          bool is_primary = (m_images_promotion_states[i] == PROMOTION_STATE_PRIMARY ||
...
void GroupPrepareImagesRequest<I>::handle_get_images_mirror_info(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " failed to get mirror images info: "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
void GroupPrepareImagesRequest<I>::check_images_child_mirroring() {
  ...
  auto ctx = create_context_callback<
    GroupPrepareImagesRequest<I>,
    &GroupPrepareImagesRequest<I>::handle_check_images_child_mirroring>(this);
  ...
  for (size_t i = 0; i < m_image_ctxs.size(); i++) {
    ...
    for (const auto& info : ictx->snap_info) {
      ...
      if (r < 0) {
        lderr (image_cct) << "failed to list children of image_id="
      ...
      for (auto &child_image : child_images) {
        ...
        if (child_pool_id == -1 ||
          ...
          r = librbd::util::create_ioctx(ictx->md_ctx, "child image",
                                         ...
                                         &child_io_ctx);
          if (r < 0) {
            lderr (image_cct) << "failed to create_ioctx of child_image_id="
                              << child_image.image_id << " of image_id="
                              << m_images[i].spec.image_id << dendl;
            auto sub = gather_ctx->new_sub();
            sub->complete(r);
            gather_ctx->activate();
        ...
        auto bl = std::make_shared<bufferlist>();
        m_child_outbls.emplace_back(bl);

        librados::ObjectReadOperation op;
        cls_client::mirror_image_get_start(&op, child_image.image_id);

        auto on_mirror_image_get = new LambdaContext(
          [this, i, bl, new_sub_ctx=gather_ctx->new_sub()](int r) {
            if (r == 0) {
              cls::rbd::MirrorImage child_mirror_image_internal;
              auto iter = bl->cbegin();
              r = cls_client::mirror_image_get_finish(&iter, &child_mirror_image_internal);
            }

            if (r == -ENOENT) {
              // child is disabled for mirroring as required
              r = 0;
            } else if (r == 0) {
              lderr(m_cct) << "mirroring is not disabled on a child of "
                           << "image_id=" << m_images[i].spec.image_id << dendl;
              r = -EBUSY;
            } else {
              lderr(m_cct) << "failed to get mirror image info for a child of "
                           << "image_id=" << m_images[i].spec.image_id << dendl;
            }
        ...
        auto comp = create_rados_callback(on_mirror_image_get);
        r = child_io_ctx.aio_operate(RBD_MIRRORING, comp, &op, bl.get());
        ceph_assert(r == 0);
        comp->release();
      }
    }
...
template <typename I>
void GroupPrepareImagesRequest<I>::handle_check_images_child_mirroring(int r) {
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " child mirroring validation for images failed: "
                 << cpp_strerror(r) << dendl;
    finish(r);
    return;
  }
...
void GroupPrepareImagesRequest<I>::update_images_read_only_mask() {
  ...
  for (size_t i = 0; i < m_image_ctxs.size(); i++) {
    {
      std::unique_lock locker{m_image_ctxs[i]->image_lock};
...
void GroupPrepareImagesRequest<I>::handle_update_images_read_only_mask(int r) {
  ...
  if (r < 0) {
    lderr(m_cct) << "group_id=" << m_group_id << ", op="
                 << m_operation << " failed to update images read only mask: "
                 << cpp_strerror(r) << dendl;
  }
