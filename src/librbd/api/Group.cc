...
int Group<I>::remove(librados::IoCtx& io_ctx, const char *group_name)
{
  ...
  if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
    ...
    lderr(cct) << "group is not primary, cannot remove" << dendl;
    return -EINVAL;
  }
                     ...
                     const char *dest_name)
{
  ...
  if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
    ...
    lderr(cct) << "group is not primary, cannot rename" << dendl;
    return -EINVAL;
  }
...
int Group<I>::snap_create(librados::IoCtx& group_ioctx,
  ...
  if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
    ...
    lderr(cct) << "group is not primary, cannot create snapshot" << dendl;
    return -EINVAL;
  }
			  ...
			  const char *snap_name)
{
  ...
  if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
    ...
    lderr(cct) << "group is not primary, cannot remove snapshot" << dendl;
    return -EINVAL;
  }
...
int Group<I>::snap_rename(librados::IoCtx& group_ioctx, const char *group_name,
  ...
  if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
    ...
    lderr(cct) << "group is not primary, cannot rename snapshot" << dendl;
    return -EINVAL;
  }
                            ...
                            ProgressContext& pctx, bool restrict_to_primary)
{
  ...
  if (restrict_to_primary) {
    ...
    if (r < 0 && r != -ENOENT && r != -EOPNOTSUPP) {
      ...
      lderr(cct) << "group is not primary, cannot perform rollback" << dendl;
      return -EINVAL;
    }
  }
