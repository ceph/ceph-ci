...
struct JSONDecoderErr : std::runtime_error {
  using runtime_error::runtime_error;
};

namespace ceph_json {
// Avoid referring to the library type in our user-facing code:
using value = glz::generic_u64;

namespace detail {

inline std::string_view as_string_view(ceph::buffer::list& bl)
{
  auto out = std::string_view { bl.c_str(), bl.length() };
  if (out.ends_with('\0')) {
    out.remove_suffix(1);
  }
...
inline void read_json_or_throw(std::string_view input, value& out)
{
  if (auto ec = glz::read_json(out, input); ec) {
    throw JSONDecoderErr(glz::format_error(ec, input));
  }
}

} // namespace detail

/* Direct API: avoids constructing the legacy JSONObj compatibility tree. */
inline bool parse(std::string_view input, value& out)
{
  return !glz::read_json(out, input);
}

inline value parse(std::string_view input)
{
  value out;
  detail::read_json_or_throw(input, out);

  return out;
}

inline bool parse(ceph::buffer::list& bl, value& out)
{
  return parse(detail::as_string_view(bl), out);
}

inline value parse(ceph::buffer::list& bl)
{
  return parse(detail::as_string_view(bl));
}

inline const value::object_t *object(const value& json) noexcept
{
  return json.get_if<value::object_t>();
}

inline const value::array_t *array(const value& json) noexcept
{
  return json.get_if<value::array_t>();
}

inline std::span<const value> array_view(const value::array_t& values) noexcept
{
  return values;
}

inline std::span<const value> array_view(const value& json) noexcept
{
  const value::array_t *values = array(json);

  return values ? array_view(*values) : std::span<const value> {};
}

inline const value *find_value(const value& json, std::string_view name) noexcept
{
  const value::object_t *members = object(json);
  if (!members) {
    return nullptr;
  }

  const auto i = members->find(name);

  return std::end(*members) == i ? nullptr : &i->second;
}

template <typename ValueT>
void decode_value(const value& json, ValueT& out)
{
  if (auto ec = glz::read_json(out, json); ec) {
    throw JSONDecoderErr(glz::format_error(ec));
  }
}

template <std::default_initializable ValueT>
ValueT decode_value(const value& json)
{
  ValueT out;
  decode_value(json, out);

  return out;
}

template <std::default_initializable ValueT>
std::optional<ValueT> field(const value& json, std::string_view name)
{
  const value *item = find_value(json, name);
  if (!item) {
    return std::nullopt;
  }

  try {
    return decode_value<ValueT>(*item);
  } catch (const JSONDecoderErr& e) {
    throw JSONDecoderErr(fmt::format("{}: {}", name, e.what()));
  }
}

template <std::default_initializable ValueT>
ValueT require(const value& json, std::string_view name)
{
  auto out = field<ValueT>(json, name);
  if (out) {
    return *out;
  }

  throw JSONDecoderErr(fmt::format("missing mandatory field {}", name));
}

inline const value::object_t& require_object(const value& json)
{
  const value::object_t *out = object(json);
  if (!out) {
    throw JSONDecoderErr("expected object");
  }

  return *out;
}

inline const value& require_object(const value& json, std::string_view name)
{
  const value *out = find_value(json, name);
  if (!out) {
    throw JSONDecoderErr(fmt::format("missing mandatory field {}", name));
  }
  if (!out->is_object()) {
    throw JSONDecoderErr(fmt::format("{}: expected object", name));
  }

  return *out;
}

inline std::span<const value> require_array(const value& json)
{
  const value::array_t *out = array(json);
  if (!out) {
    throw JSONDecoderErr("expected array");
  }

  return array_view(*out);
}
                                            ...
                                            std::string_view name)
{
  ...
  const value::array_t *values = array(*out);
  if (!values) {
    throw JSONDecoderErr(fmt::format("{}: expected array", name));
  }

  return array_view(*values);
}
...
/* Note: std::is_integer<> will also pick up bool, which in our
...
From the extant code, the same rules should be applied to char, etc., 
so I've also done so here:
*/
template <typename T>
...
class JSONObjIter final {
  ...
  using map_iter_t = boost::container::flat_multimap<std::string, std::unique_ptr<JSONObj>, std::less<>>::iterator;
...
protected:
  ...
  void handle_value(const ceph_json::value& v);

protected:
  static bool parse_json(std::string_view input, ceph_json::value& data_out) {
    return !glz::read_json(data_out, input);
  }

public:
  JSONObj() = default;

  JSONObj(JSONObj *parent_node, std::string_view name_in, const ceph_json::value& data_in)
   : parent { parent_node },
     name { name_in },
     data(data_in)
  {
	...
	if (data_in.is_string())
         val.set(data_in.as<std::string>(), true);
	else {
         std::string json_data;
	 if(auto ec = glz::write_json(data, json_data); ec) {
          throw JSONDecoderErr(fmt::format("failed to parse JSON input: {}", glz::format_error(ec, name)));
         }
         val.set(json_data, false);
        }
...
public:
  ...
  bool get_attr(std::string_view attr_name, data_val& attr) {
	if (name != attr_name)
	 return false;
