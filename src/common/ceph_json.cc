...
void JSONObj::handle_value(const ceph_json::value& v)
{
  if (auto *o = v.get_if<ceph_json::value::object_t>()) {
    ...
    for (const auto& [k, v] : *o) {
      auto child = std::make_unique<JSONObj>(this, k, v);
      children.emplace(std::pair { k, std::move(child) });
    }
 ...
 if (auto *ap = v.get_if<ceph_json::value::array_t>()) {
      ...
      for (const auto& kv : *ap) {
        auto child = std::make_unique<JSONObj>(this, "", kv);
...
{
 auto *elements = data.get_if<ceph_json::value::array_t>();

 if(!elements)
  return {}; 

 vector<string> decoded_elements;
 decoded_elements.reserve(elements->size());

 std::ranges::transform(*elements, std::back_inserter(decoded_elements), [](const auto& e) {
    auto json_str = glz::write_json(e);
...
bool JSONParser::parse(std::string_view json_string_view) 
{
  ...
  if (data.is_string()) {
    val.set(data.get<std::string>(), true);
    return true;
  } 
