...
typedef vector<pair<string_view,string_view>> perf_counter_label_pairs;
    ...
    const perf_counter_label_pairs &labels) const
{
  ...
  */
  ...
  // NOTE: This should ideally use the resolved_path, but since the resolved path
  // contains NULL delimiters and we cannot have an array section whose key has
  // NULL characters, we will just use the counter_name to extract the value at
  // the call site
  ...
  if (!labels.empty()){
    ...
    std::string counter_name_with_labels = ceph::perf_counters::detail::create(counter_name.data(), perf_counter_labels.data(), perf_counter_labels.data() + perf_counter_labels.size());
    resolved_path = std::string(counter_name_with_labels) + "." + std::string(sub_counter_name);
  } else {
    resolved_path = std::string(counter_name) + "." + std::string(sub_counter_name);
  }

  {
    ...
    if (metadata) {
      ...
      if (metadata->perf_counters.instances.count(resolved_path)) {
        ...
        dout(4) << fmt::format("Missing counter: '{}' ({}.{})", resolved_path, svc_name, svc_id) << dendl;
        dout(20) << "Paths are:" << dendl;
        for (const auto &i : metadata->perf_counters.instances) {
          dout(20) << i.first << dendl;
        }
      ...
      dout(4) << fmt::format("No daemon state for {}.{}", svc_name, svc_id) << dendl;
    }
  }
  return f.get();
}
    ...
    const perf_counter_label_pairs &labels)
{
  ...
  return with_perf_counters(extract_latest_counters, svc_name, svc_id, counter_name, sub_counter_name, labels);
}
  ...
  if (!daemons.empty()) {
    for (auto& [key, state] : daemons) {
      ...
      with_gil(no_gil, [&, key=ceph::to_string(key), state=state] {
        ...
        for (const auto& [counter_name_with_labels, _] : state->perf_counters.instances) {
          /*
          ...
          */
          ...
          auto labels = ceph::perf_counters::key_labels(counter_name_with_labels);
          std::copy_if(labels.begin(), labels.end(), std::back_inserter(key_labels),
        ...
        if (prev_key_name != key_name){
          ...
          dout(4) << fmt::format("{} unable to create perf schema, not a valid condition", __func__) << dendl;
        }
      }
      f.close_section(); // close 'counters'
      f.close_section(); // close 'counter object' section
    ...
    dout(4) << fmt::format("{}: No daemon state found for  {}.{}", __func__, svc_type, svc_id) << dendl;
  }
  return f.get();
}
