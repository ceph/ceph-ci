...
void NVMeofGwMonitorClient::handle_nvmeof_gw_map(ceph::ref_t<MNVMeofGwMap> nmap)
{
  ...
  auto reset_elapsed_seconds = std::chrono::duration_cast<std::chrono::seconds>(now - reset_timestamp).count();
  NvmeGwClientState old_gw_state;
  ...
  */
  if ( (reset_elapsed_seconds < 15) && !got_new_gw_state && got_old_gw_state) {
