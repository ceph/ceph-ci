...
void MPRadosSerializer::stop_renewal()
{
  ...
  cond.async_wait(lock, rgw::oyc(dpp, y, ec_ignored));
}
...
int RadosRestore::push(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  try {
    fifos[index]->push(dpp, items, rgw::oyc(dpp, y));
  } catch (const sys::system_error& e) {
...
int RadosRestore::push(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  try {
    fifos[index]->push(dpp, std::move(bl), rgw::oyc(dpp, y));
  } catch (const sys::system_error& e) {
    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__
		 << ": unable to push to FIFO: " << obj_names[index]
		 << ": " << e.what() << dendl;
...
int RadosRestore::trim(const DoutPrefixProvider *dpp, optional_yield y,
  ...
  try {
    fifos[index]->trim(dpp, std::string(marker), false, rgw::oyc(dpp, y));
  } catch (const sys::system_error& e) {
    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__
