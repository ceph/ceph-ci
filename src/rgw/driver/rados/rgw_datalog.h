...
class RGWDataChangesLog {
  ...
  std::vector<std::unordered_set<std::string>> semaphores{unsigned(num_shards)};
...
public:
  ...
  int trim_generations(const DoutPrefixProvider *dpp,
		       std::optional<uint64_t>& through,
		       optional_yield y);
  asio::awaitable<void> read_all_sems(
    int index, std::unordered_map<std::string, uint64_t>* out);
  asio::awaitable<bool>
  synthesize_entries(const DoutPrefixProvider* dpp, int index,
		     const std::unordered_map<std::string, uint64_t>& semcount);
  asio::awaitable<bool>
