...
struct rgw_http_req_data : public RefCountedObject {
        ...
        [this, &lock] (auto handler, auto ex) {
          ...
          if (cancel_slot.is_connected()) {
            cancel_slot.assign([this]
              ...
              /*
               * This handler runs inline on whatever thread emits the
               * cancellation signal; cancellation slots have no internal
               * synchronization. It is safe only while every emitter is
               * serialized with the waiter's executor, which orders the
               * emit against cancel_slot.clear() in wait() and keeps the
               * caller's req_data ref alive while the handler runs. The
               * restore worker posts its emits onto the waiter's strand
               * for this reason. Snapshot under the lock, then call
               * remove_request() without it (reqs_lock is ordered before
               * lock).
               */
  ...
  int wait(const DoutPrefixProvider* dpp, optional_yield y) {
    ...
    if (y) {
      ...
      if (cancel_slot.is_connected()) {
        cancel_slot.clear();   // emitters are serialized with this executor, so the handler is not running
      }
      return -ec.value();
    }
