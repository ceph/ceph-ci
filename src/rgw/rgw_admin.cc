...
void usage()
{
  ...
  cout << "  topic migrate                    migrate topic and notifications "
          "for a given rgw_user to account-id\n";
  cout << "options:\n";
...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_DELETE) {
  ...
  if (opt_cmd == OPT::PUBSUB_TOPIC_MIGRATE) {
    ...
    std::string next_token = marker;
    const std::optional<rgw_owner> userid = user->get_id();
    int final_ret = 0;
    do {
      rgw_pubsub_topics result;
      ...
      for (const auto& [_, old_topic] : result.topics) {
        ...
        if (old_topic.dest.persistent) {
          ...
          if (ret < 0) {
            cerr << "Failed to create persistent queue for topic: "
        ...
        for (const auto& bucket_key : subscribed_buckets) {
          ...
          std::ignore = rgw_parse_url_bucket(bucket_key, tenant, tenant_name,
                                             bucket_name);
          ...
          // finally update the bucket notifications attr.
          bufferlist bl;
          bucket_topics.encode(bl);
          ret = retry_raced_bucket_write(
              dpp(), bucket.get(),
              [&bl, bucket = std::move(bucket)] {
                rgw::sal::Attrs& attrs = bucket->get_attrs();
                attrs[RGW_ATTR_BUCKET_NOTIFICATION] = bl;
                return bucket->merge_and_store_attrs(dpp(), attrs, null_yield);
              },
              null_yield);
          if (ret < 0) {
