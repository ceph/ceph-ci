...
static SimpleCmd::Commands all_cmds = {
  ...
  { "datalog semaphore reset", OPT::DATALOG_SEMAPHORE_LIST },
...
#define MAX_LC_LIST_ENTRIES 100
  ...
  if (opt_cmd == OPT::DATALOG_SEMAPHORE_LIST) {
    if (max_entries < 0) {
      // I hate this. Unspecified should be unlimited. But it makes
      // sense to follow the rest.
      max_entries = 1000;
    }
    ...
    ret = run_coro(datalog->admin_sem_list(shard, max_entries, marker, *formatter),
