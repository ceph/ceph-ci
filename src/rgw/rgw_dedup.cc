...
namespace rgw::dedup {
        ...
        if (n == sizeof(disk_block_t)) {
          ...
          for (unsigned rec_id = 0; rec_id < p_header->rec_count; rec_id++) {
            ...
            if (step == STEP_BUILD_TABLE) {
              add_record_to_dedup_table(p_table, &rec, disk_block_id, rec_id, p_stats);
              slab_rec_count++;
            }
#ifdef FULL_DEDUP_SUPPORT
                                                   ...
                                                   worker_stats_t             *p_worker_stats /*IN-OUT*/)
  {
    ...
    // TBD: what should be done about storage_class and hwo do we check for this ??
    // Do we need to check for any empty storage_class or should we accept STANDARD?
    if (entry.meta.storage_class.empty() || entry.meta.storage_class == "STANDARD"){
      p_worker_stats->default_storage_class_objs++;
      p_worker_stats->default_storage_class_objs_bytes += ondisk_byte_size;
    }
    else {
      ldpp_dout(dpp, 10) << __func__ << "::" << entry.key.name
  ...
  {
    uint64_t num_rados_objects_bytes = 0;
    collect_pool_stats(dpp, rados, &d_num_rados_objects, &num_rados_objects_bytes);
