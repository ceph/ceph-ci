...
int PeerReplayer::copy_to_remote(std::shared_ptr<SyncEntry> &cur_entry,
  ...
  dout(0) << ": checking len1 -->" << cur_entry->epath << ", " << b->len << ", "
          << cur_entry->stx.stx_size << dendl;
  ...
  if (num_blocks == 0 && r >= 0) { // handle blocklist case
    ...
    dout(0) << ": checking len2 -->" << cur_entry->epath << ", "
            << cur_entry->stx.stx_size << ", " << r << dendl;
    if (r < 0) {
...
close_local_fd:
  ...
  dout(0) << ": file transfer done-->" << cur_entry->epath << ", "
          << cur_entry->sync_is_snapdiff()
          << (cur_entry->sync_is_snapdiff() ? " found" : "") << dendl;
  return r == 0 ? 0 : r;
}
...
int PeerReplayer::DirBruteDiffSync::sync_current_entry() {
  // dout(0) << "brute dir sync-->" << cur_entry->epath << dendl;
  int r = 0, rem_r = 0;
  r = populate_current_stat(fh);
  if (r < 0) {
    return r;
