...
std::string_view JournalTool::get_event_name_str(unsigned int event_type)
{
  switch (event_type) {
    ...
    case EVENT_SEGMENT:         return "EVENT_SEGMENT";
...
int JournalTool::recover_header(bool dry_run)
{
  ...
  if (!js.header_valid) {
    ...
    if (js.header->magic != CEPH_FS_ONDISK_MAGIC) {
      derr << "Critical failure: Invalid journal magic number (0x"
           << std::hex << js.header->magic << std::dec
           << "). This does not appear to be a valid CephFS journal." << dendl;
      return -EINVAL;
    }
