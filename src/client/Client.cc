...
int Client::path_walk(InodeRef dirinode, const filepath& origpath, walk_dentry_result* result, const UserPerm& perms, const PathWalk_ExtraOptions& extra_options)
{
  ...
  while (i < path.depth() && diri) {
    ...
    int r = _lookup(diri, dname, caps, &next, perms, extra_options.is_rename);
    if (r == -CEPHFS_ENOENT && i == (path.depth()-1) && !extra_options.require_target) {
      target = InodeRef();
      return 0;
    } else if (r < 0) {
      return r;
		   ...
		   const UserPerm& perms, InodeRef *inp)
{
  ...
  walk_dentry_result wdr;
  if (int rc = path_walk(dir, filepath(name), &wdr, perms, {.require_target = false}); rc == 0 && wdr.target) {
    return -CEPHFS_EEXIST;
                   ...
                   std::string alternate_name)
{
  ...
  walk_dentry_result wdr;
  if (int rc = path_walk(dir, filepath(name), &wdr, perm, {.require_target = false}); rc == 0 && wdr.target) {
    return -CEPHFS_EEXIST;
  } else if (rc != -CEPHFS_ENOENT) {
    return rc;
  }
...
int Client::_rename(Inode *fromdir, const char *fromname, Inode *todir, const char *toname, const UserPerm& perm, std::string alternate_name)
{
  ...
  if (int rc = path_walk(fromdir, filepath(fromname), &wdr_from, perm, {.is_rename = true}); rc < 0) {
    return rc;
  }
