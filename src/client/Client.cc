		  ...
		  const UserPerm& perms, frag_info_t *dirstat, int mask)
{
  ...
  if (int rc = path_walk(cwd, path, &in, perms, false, mask); rc < 0) {
    return rc;
  }
  if (int rc = _getattr(in, mask, perms); rc < 0) {
    ldout(cct, 3) << __func__ << " exit on error!" << dendl;
...
int Client::chmodat(int dirfd, const char *relpath, mode_t mode, int flags,
  ...
  if (int rc = path_walk(dirinode, path, &in, perms, !(flags & AT_SYMLINK_NOFOLLOW), 0); rc < 0) {
    return rc;
  }
  struct stat attr;
  attr.st_mode = mode;
...
int Client::chownat(int dirfd, const char *relpath, uid_t new_uid, gid_t new_gid,
  ...
  if (int rc = path_walk(dirinode, relpath, &in, perms, !(flags & AT_SYMLINK_NOFOLLOW), 0); rc < 0) {
    return rc;
  }
                    ...
                    const UserPerm& perms)
{
  ...
  std::scoped_lock lock(client_lock);
  if (int rc = path_walk(cwd, path, &in, perms, false); rc < 0) {
    return rc;
  }
  struct ceph_statx attr;
  utime_t(times[0]).to_timespec(&attr.stx_atime);
...
int Client::statxat(int dirfd, const char *relpath,
  ...
  if (int rc = path_walk(dirinode, filepath(relpath), &in, perms, !(flags & AT_SYMLINK_NOFOLLOW), mask); rc < 0) {
    return rc;
  }
