...
public:
  ...
  laddr_t get_metadata_prefix() const {
    auto ret = *this;
    ret.value &= layout::MD_PREFIX_MASK;
    return ret;
  }
...
private:
  ...
  struct FieldSpec {
    ...
    static ReturnType get(const Unsigned &laddr_value) {
      static constexpr uint16_t SHIFT =
        std::numeric_limits<uint64_t>::digits - length;
      if constexpr (std::is_signed_v<ReturnType>) {
        Unsigned v = (laddr_value & MASK) >> OFFSET;
        int64_t signed_v = static_cast<int64_t>(v << SHIFT) >> SHIFT;
        return static_cast<ReturnType>(signed_v);
      } else {
  ...
  struct layout_v1 {
    ...
    static constexpr Unsigned MD_PREFIX_MASK =
	CLONE_PREFIX_MASK | MetadataFlagSpec::MASK;
  };
