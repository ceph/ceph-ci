  ...
  ECListener& pg)
{
  ...
  if (op.backfill_or_async_recovery) {
    for (const auto& obj : op.temp_removed) {
      INFODPP("hoid={} removing object since we won't get the transaction",
	      dpp, obj);
  ...
  if (pg.is_missing_object(op.soid)) {
    async = true;
    DEBUGDPP("hoid={} is missing", dpp, op.soid);
  ...
  const uint32_t flags)
{
  ...
  DEBUGDPP("hoid={} off={} size={} flags={}", dpp, obj, off, size, flags);
  DEBUGDPP("ghobj={}", dpp, ghobject_t{obj, ghobject_t::NO_GEN, get_shard()});
...
ECBackend::handle_rep_read_op(ECSubRead& op)
{
  ...
  return seastar::do_with(ECSubReadReply{},
    ...
    return interruptor::do_for_each(op.to_read, [FNAME, &op, &reply, this] (auto read_item) {
	  ...
	  DEBUGDPP("hoid={} requested={} len={}", dpp, obj, size, result_bl.length());
	  ...
	  if (e.value() == ENOENT && fast_read) {
	    INFODPP("hoid={} ENOENT reading, fast read, probably ok", dpp, obj);
	  } else {
	    ERRORDPP("hoid={} error={} reading", dpp, obj, e.value());
	...
	DEBUGDPP("hoid={} fulfilling attr request", dpp, obj_attr);
...
ECBackend::handle_rep_read_reply(ECSubReadReply& mop)
{
  ...
  for (auto &&[hoid, err]: mop.errors) {
    ...
    DEBUGDPP("hoid={} shard={} error={}", dpp, hoid, from, err);
  }
  {
  ...
  if (rop.do_redundant_reads || all_sub_reads_done) {
    for (auto &&[oid, read_result]: rop.complete) {
      ...
      if (err) {
	DEBUGDPP("tid={} hoid={} minimum_to_decode failed err={}", dpp, rop.tid, oid, err);
        if (all_sub_reads_done) {
	  ...
	  if (local_conf()->osd_read_ec_check_for_errors) {
	    INFODPP("hoid={} not ignoring errors, use one shard err={}",
		    dpp, oid, err);
            err = rop.complete.at(oid).errors.begin()->second;
            rop.complete.at(oid).r = err;
	  } else {
	    INFODPP("hoid={} error(s) ignored, enough copies available",
		    dpp, oid);
