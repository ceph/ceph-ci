...
seastar::future<> PGShardManager::prime_merges(epoch_t first, epoch_t last)
{
  ...
  for (epoch_t e = std::max<epoch_t>(first, 1); e <= last; ++e) {
    ...
    for (const auto& [poolid, pool] : cur->get_pools()) {
      // PG merge is a crimson-only feature; classic pools advance elsewhere.
      if (!pool.is_crimson() || !prev->have_pg_pool(poolid)) {
        continue;
      }
      ...
      // Replicated pools only (NO_SHARD); for EC the shard would need to come
      // from the acting set, which is left as future work.
  ...
  // through the merge epoch.  Placement is left to the normal load-balancing
  // mapping; a genuinely cross-shard (cross-core) merge on seastore is handled
  // by ShardServices::seastore_merge_shards_ok()'s abort path, not worked
  // around here.
  for (const auto& [pgid, merge_epoch] : merge_pgs) {
    auto mapping = co_await get_pg_to_shard_mapping().get_or_create_pg_mapping(pgid);
    const core_id_t core = mapping.first;
    const store_index_t store_index = mapping.second;
