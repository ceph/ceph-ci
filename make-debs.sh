...
# Authoritatively rewrite DEB_BUILD_PROFILES: strip pkg.ceph.crimson if present,
# then add it back only when we've decided to enable crimson. Other profiles
# (e.g. noudeb) are preserved. This ensures a pre-set DEB_BUILD_PROFILES from
# the CI environment can't sneak crimson past the distro gate.
new_profiles=
for p in $DEB_BUILD_PROFILES; do
    [ "$p" = pkg.ceph.crimson ] && continue
    new_profiles="${new_profiles:+$new_profiles }$p"
done
if [ "$enable_crimson" = yes ]; then
    new_profiles="${new_profiles:+$new_profiles }pkg.ceph.crimson"
fi
export DEB_BUILD_PROFILES="$new_profiles"
